...
func (r *themeRepo) GetByID(id int) (model.Theme, error) {
	...
	if err := r.db.Get(&row, `SELECT id, name FROM themes WHERE id = $1`, id); err != nil {
		return model.Theme{}, fmt.Errorf("themeRepo.GetByID: %w", wrapNotFound(err))
	}
	return row.toDomain(), nil
}

func (r *themeRepo) Create(t model.Theme) (model.Theme, error) {
	row := toThemeDB(t)
	err := r.db.QueryRowx(
...
func (r *themeRepo) Update(id int, t model.Theme) (model.Theme, error) {
	row := toThemeDB(t)
	row.ID = id
	res, err := r.db.Exec(
...
func (r *themeRepo) Delete(id int) error {
	res, err := r.db.Exec(`DELETE FROM themes WHERE id=$1`, id)
	if err != nil {
